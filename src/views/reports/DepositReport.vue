<template>
  <div class="space-y-5">
    ...
    <div v-if="showMoreFilters" class="card p-4 grid grid-cols-2 sm:grid-cols-3 gap-3">
      ...
      <div><label class="label text-xs">Client</label><SearchableSelect v-model="filters.client_id" :options="clients" placeholder="All clients" @update:modelValue="load" /></div>
      ...
      <div><label class="label text-xs">Bonus Option</label><SearchableSelect v-model="filters.bonus_option_id" :options="lookup.bonusOptions" placeholder="All bonus" @update:modelValue="load" /></div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
      ...
      <div class="card p-4">
        ...
        <p class="text-2xl font-bold text-green-600 mt-1">{{ items.filter(d=>d.status==='approved').length }}</p>
      </div>
      <div class="card p-4">
        <p class="text-xs text-gray-500">Pending</p>
        <p class="text-2xl font-bold text-amber-600 mt-1">{{ items.filter(d=>d.status==='pending').length }}</p>
      </div>
    </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full">
          ...
          <tbody class="divide-y divide-gray-50">
            <tr v-if="loading"><td colspan="21" class="table-cell text-center py-10 text-gray-400">Loading…</td></tr>
            <tr v-else-if="!items.length"><td colspan="21" class="table-cell text-center py-10 text-gray-400">No records</td></tr>
            <tr v-else v-for="(row, i) in items" :key="row.id" class="hover:bg-gray-50/60">
              ...
              <td class="table-cell font-mono text-xs font-semibold text-gray-600">{{ row.transaction_no }}</td>
              ...
              <td class="table-cell text-green-600 text-sm whitespace-nowrap">+{{ fmt(row.bonus_amount, row.currency) }}</td>
              <td class="table-cell font-bold text-sm whitespace-nowrap" style="color:#938af4">{{ fmt(row.bal, row.currency) }}</td>
              <td class="table-cell text-sm whitespace-nowrap">{{ fmt(row.to, row.currency) }}</td>
              <td class="table-cell text-sm whitespace-nowrap">{{ fmt(row.os, row.currency) }}</td>
            ...
            <tr v-if="items.length" class="bg-gray-50 font-semibold border-t-2 border-gray-200">
              <td colspan="10" class="table-cell text-right text-xs text-gray-500 pr-3">Totals</td>
              <td class="table-cell whitespace-nowrap">
                <p class="text-xs text-indigo-600">${{ totalUSD }}</p>
                <p class="text-xs text-purple-600">៛{{ totalKHR }}</p>
              </td>
              <td class="table-cell"></td>
              <td class="table-cell"></td>
              <td class="table-cell whitespace-nowrap">
                <p class="text-xs text-green-600">+${{ bonusTotalUSD }}</p>
                <p class="text-xs text-purple-600">+៛{{ bonusTotalKHR }}</p>
              </td>
              <td class="table-cell whitespace-nowrap">
                <p class="text-xs" style="color:#938af4">${{ balTotalUSD }}</p>
                <p class="text-xs text-purple-600">៛{{ balTotalKHR }}</p>
              </td>
              <td class="table-cell whitespace-nowrap">
                <p class="text-xs text-gray-700">${{ toTotalUSD }}</p>
                <p class="text-xs text-purple-600">៛{{ toTotalKHR }}</p>
              </td>
              <td class="table-cell whitespace-nowrap">
                <p class="text-xs text-gray-700">${{ osTotalUSD }}</p>
                <p class="text-xs text-purple-600">៛{{ osTotalKHR }}</p>
              </td>
...
import { getDeposits } from '@/api/transactions'
...
const filters = ref({ date_from: todayStart(), date_to: todayEnd(), status: null, currency: null, branch_id: null, created_by_id: null, client_id: null, product_type_id: null, bonus_option_id: null, approved_by_id: null })

const activeMoreFilters = computed(() => [filters.value.branch_id, filters.value.created_by_id, filters.value.client_id, filters.value.product_type_id, filters.value.bonus_option_id, filters.value.approved_by_id].filter(Boolean).length)
const statusOpts   = [{ id:'pending',name:'Pending' }, { id:'approved',name:'Approved' }, { id:'rejected',name:'Rejected' }]
const currencyOpts = [{ id:'USD', name:'USD — US Dollar' }, { id:'KHR', name:'KHR — Cambodian Riel' }]

const totalUSD       = computed(() => formatNumber(sumByCurrency(items.value, 'USD', 'amount')))
const totalKHR       = computed(() => formatNumber(sumByCurrency(items.value, 'KHR', 'amount')))
const bonusTotalUSD  = computed(() => formatNumber(sumByCurrency(items.value, 'USD', 'bonus_amount')))
const bonusTotalKHR  = computed(() => formatNumber(sumByCurrency(items.value, 'KHR', 'bonus_amount')))
const balTotalUSD    = computed(() => formatNumber(sumByCurrency(items.value, 'USD', 'bal')))
const balTotalKHR    = computed(() => formatNumber(sumByCurrency(items.value, 'KHR', 'bal')))
const toTotalUSD     = computed(() => formatNumber(sumByCurrency(items.value, 'USD', 'to')))
const toTotalKHR     = computed(() => formatNumber(sumByCurrency(items.value, 'KHR', 'to')))
const osTotalUSD     = computed(() => formatNumber(sumByCurrency(items.value, 'USD', 'os')))
const osTotalKHR     = computed(() => formatNumber(sumByCurrency(items.value, 'KHR', 'os')))
...
function resetFilters() { filters.value = { date_from:todayStart(), date_to:todayEnd(), status:null, currency:null, branch_id:null, created_by_id:null, client_id:null, product_type_id:null, bonus_option_id:null, approved_by_id:null }; load() }

function exportCSV() {
  const headers = ['#','Date','Transaction No','Created By','Client','Client Bank','Product','Account Id','Company Bank','Amount','Currency','Bonus Option','Bonus','Bal','TO','OS','Status','Approved Date','Approved By','Remark']
  const rows = items.value.map((r, i) => [
    i+1, fmtDate(r.date), r.transaction_no, r.created_by?.name||'',
    r.client?.name||'', r.client_bank?.bank_type?.name||'', r.client_product?.product_type?.name||'',
    r.client_product?.account_id||'', r.company_bank?.name||'',
    r.amount, r.currency, r.bonus_option?.name||'', r.bonus_amount, r.bal, r.to, r.os,
    r.status, fmtDate(r.approved_at), r.approved_by?.name||'', r.remark||''
  ])
  const csv = [headers, ...rows].map(r => r.map(v => `"${String(v).replace(/"/g,'""')}"`).join(',')).join('\n')
  const a = document.createElement('a'); a.href = 'data:text/csv;charset=utf-8,' + encodeURIComponent(csv); a.download = 'deposits_report.csv'; a.click()
}

onMounted(async () => { load(); lookup.loadAll(); try { const r = await getBranches({ show_all: false }); branches.value = (r.data||[]).map(b=>({id:b.id,name:b.name,sub:b.code})) } catch {} try { const r = await getUsersInScope(); users.value = (r.data||[]).map(u=>({id:u.id,name:u.name,sub:u.email})) } catch {} try { const r = await getClients({ page:1, page_size:500 }); clients.value = (r.data||[]).map(c=>({id:c.id,name:c.name,sub:c.code})) } catch {} try { const r = await getUsersInScope(); approvedByUsers.value = (r.data||[]).map(u=>({id:u.id,name:u.name,sub:u.email})) } catch {} })
</script>