  ...
  <div class="space-y-5">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
      ...
      <div class="card p-4">
        ...
        <p class="text-base font-bold mt-1 text-orange-600">-${{ totalUSD }}</p>
        <p class="text-base font-bold mt-0.5 text-blue-600">-៛{{ totalKHR }}</p>
      </div>
      <div class="card p-4">
        <p class="text-xs text-gray-500">Approved</p>
        <p class="text-2xl font-bold text-green-600 mt-1">{{ items.filter(w=>w.status==='approved').length }}</p>
      </div>
      <div class="card p-4">
        <p class="text-xs text-gray-500">Pending</p>
        <p class="text-2xl font-bold text-amber-600 mt-1">{{ items.filter(w=>w.status==='pending').length }}</p>
      </div>
    </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full">
          ...
          <tbody class="divide-y divide-gray-50">
            <tr v-if="loading"><td colspan="19" class="table-cell text-center py-10 text-gray-400">Loading…</td></tr>
            <tr v-else-if="!items.length"><td colspan="19" class="table-cell text-center py-10 text-gray-400">No records</td></tr>
            <tr v-else v-for="(row, i) in items" :key="row.id" class="hover:bg-gray-50/60">
              ...
              <td class="table-cell font-semibold text-orange-600 whitespace-nowrap">-{{ fmt(row.amount, row.currency) }}</td>
              <td class="table-cell">
                <span :class="['badge text-xs', row.currency==='USD' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700']">{{ row.currency }}</span>
              </td>
              <td class="table-cell text-sm whitespace-nowrap" :class="row.bal<0?'text-red-600':'text-gray-700'">{{ fmt(row.bal, row.currency) }}</td>
            ...
            <tr v-if="items.length" class="bg-gray-50 font-semibold border-t-2 border-gray-200">
              <td colspan="10"  class="table-cell text-right text-sm text-gray-600">Totals</td>
              <td class="table-cell whitespace-nowrap">
                <p class="block text-orange-600">-${{ totalUSD }}</p>
                <p class="block text-blue-600">-៛{{ totalKHR }}</p>
              </td>
              <td class="table-cell"></td>
              <td class="table-cell whitespace-nowrap">
                <p class="block" style="color:#938af4">${{ balTotalUSD }}</p>
                <p class="block text-purple-600">៛{{ balTotalKHR }}</p>
              </td>
              <td class="table-cell whitespace-nowrap">
                <p class="block text-gray-700">${{ toTotalUSD }}</p>
                <p class="block text-purple-600">៛{{ toTotalKHR }}</p>
              </td>
              <td class="table-cell whitespace-nowrap">
                <p class="block text-gray-700">${{ osTotalUSD }}</p>
                <p class="block text-purple-600">៛{{ osTotalKHR }}</p>
              </td>
              <td colspan="9" class="table-cell"></td>
            </tr>
          </tbody>
        </table>
      </div>
...
import { getWithdrawals } from '@/api/transactions'
...
const users    = ref([])
const clients  = ref([])
const filters = ref({ date_from: todayStart(), date_to: todayEnd(), status: null, currency: null, branch_id: null, created_by_id: null, client_id: null, product_type_id: null, approved_by_id: null })

const activeMoreFilters = computed(() => [filters.value.branch_id, filters.value.created_by_id, filters.value.client_id, filters.value.product_type_id, filters.value.approved_by_id].filter(Boolean).length)
...
function resetFilters() { filters.value = { date_from:todayStart(), date_to:todayEnd(), status:null, currency:null, branch_id:null, created_by_id:null, client_id:null, product_type_id:null, approved_by_id:null }; load() }

function exportCSV() {
  const headers = ['#','Date','Transaction No','Created By','Client','Product','Account Id','Client Bank','Company Bank','Amount','Currency','Bal','TO','OS','Status','Approved Date','Approved By','Remark']
  const rows = items.value.map((r, i) => [
    i+1, fmtDate(r.date), r.transaction_no, r.created_by?.name||'',
    r.client?.name||'', r.client_product?.product_type?.name||'', r.client_product?.account_id||'',
    r.client_bank?.bank_type?.name||'', r.company_bank?.name||'',
    r.amount, r.currency, r.bal, r.to, r.os,
    r.status, fmtDate(r.approved_at), r.approved_by?.name||'', r.remark||''
  ])
  const csv = [headers, ...rows].map(r => r.map(v => `"${String(v).replace(/"/g,'""')}"`).join(',')).join('\n')
  const a = document.createElement('a'); a.href = 'data:text/csv;charset=utf-8,' + encodeURIComponent(csv); a.download = 'withdrawals_report.csv'; a.click()
}
