<template>
  <div class="max-w-4xl mx-auto space-y-5">
    <div class="flex items-center gap-3">
      ...
      <div>
        <h1 class="text-xl font-semibold text-gray-800">{{ isEdit ? 'Edit Deposit' : 'New Deposit' }}</h1>
    ...
    >
      ...
      <template #actions>
        <RouterLink to="/deposits" class="btn-secondary">Cancel</RouterLink>
        <RequiredFieldsGuard
          :fields="{
            Branch: branchId,
...
import { createDeposit, updateDeposit, getDeposit } from '@/api/transactions'
...
async function handleSubmit(formData) {
  ...
  try {
    ...
    if (isEdit.value) {
      await updateDeposit(route.params.id, {
