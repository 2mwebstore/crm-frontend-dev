<template>
  <div class="space-y-5">
    <div>
      <h1 class="text-xl font-semibold text-gray-800">Report Leave</h1>
      <p class="text-sm text-gray-500 mt-0.5">Leave by staff member for the selected period</p>
    </div>
    ...
    <div v-else-if="!groups.length" class="card p-10 text-center text-gray-400">No leave requests in this period</div>
        ...
        <table class="w-full">
          ...
          <tbody class="divide-y divide-gray-50">
            <tr v-for="g in groups" :key="g.userId" class="hover:bg-gray-50/60">
              ...
              <td class="table-cell text-sm font-medium text-primary-600">{{ g.totalDays }} day{{ g.totalDays === 1 ? '' : 's' }}</td>
            </tr>
          </tbody>
        </table>
      </div>
          ...
          <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-50">
              <tr v-for="r in g.rows" :key="r.id" class="hover:bg-gray-50/60">
                <td class="table-cell text-sm text-gray-800 whitespace-nowrap">{{ fmtDateMonth(r.date_from) }}<span v-if="r.date_to !== r.date_from"> → {{ fmtDateMonth(r.date_to) }}</span></td>
                <td class="table-cell text-sm text-gray-700">{{ r.leave_type?.name || '—' }}</td>
              ...
              <tr>
                <td colspan="5" class="table-cell text-sm text-primary-600 font-medium">Total: {{ g.totalDays }} day{{ g.totalDays === 1 ? '' : 's' }}</td>
...
import { getLeaveRequests } from '@/api/leave-requests'
...
const groups = computed(() => {
  ...
  for (const r of items.value) {
    const key = r.user_id
    if (!byUser.has(key)) {
      byUser.set(key, { userId: key, userName: r.user?.name || '—', branches: new Set(), rows: [], totalDays: 0 })
    }
    const g = byUser.get(key)
    g.rows.push(r)
    g.totalDays += Number(r.duration) || 0
