<template>
  <div class="space-y-5">
    <div>
      <h1 class="text-xl font-semibold text-gray-800">List Leave</h1>
      <p class="text-sm text-gray-500 mt-0.5">Leave requests from all staff</p>
    </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full">
          ...
          <tbody class="divide-y divide-gray-50">
            <tr v-if="loading">
              <td colspan="9" class="table-cell text-center py-10 text-gray-400">Loading…</td>
            </tr>
            <tr v-else-if="!items.length">
              <td colspan="9" class="table-cell text-center py-10 text-gray-400">No leave requests found</td>
            </tr>
            <tr v-else v-for="r in items" :key="r.id" class="hover:bg-gray-50/60">
              ...
              <td class="table-cell text-sm text-gray-600 whitespace-nowrap">{{ r.date_from }}<span v-if="r.date_to !== r.date_from"> → {{ r.date_to }}</span></td>
    ...
    <!-- Reject reason modal -->
    <AppModal v-model="rejectModal" title="Reject Leave Request" size="sm">
      <div>
...
import { getLeaveRequests, approveLeaveRequest, rejectLeaveRequest } from '@/api/leave-requests'
...
const canApprove = computed(() => auth.isSuperAdmin || auth.can('leave_requests.approve'))
...
async function doApprove(r) {
  try {
    await approveLeaveRequest(r.id)
